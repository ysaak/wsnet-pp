/**
 *  \file   stats_app_sample.c
 *  \brief  A test application with statistics. See STATS in the file.
 *  \author Romain KUNTZ
 *  \date   01/2010
 **/
...
#include "../environment/stats_handler_sample.h"

/* ************************************************** */
/* ************************************************** */
model_t model =  {
    "A test application with statistics",
    "Romain KUNTZ",
    "0.1",
...
/* ************************************************** */
/* ************************************************** */
/* STATS: node handler for the statistics at the end of 
 * the simulation */
int stats_handler(call_t *c, void *das) {
    struct _app_stat *stat = NULL;

    PRINT_STATISTICS("\n=== Node %d statistics handler ===\n",
                    c->node);

    das_init_traverse(das);
    while ((stat = (struct _app_stat *) das_traverse(das)) != NULL) {
        /* Displays packet received or sent by the node */
        if (stat->rx_time) {
            PRINT_STATISTICS("Node %d rx pkt seq. %d "
                        "at time %"PRId64"\n",
                        c->node, stat->pkt_seq, stat->rx_time);
        } else {
            PRINT_STATISTICS("Node %d tx pkt seq. %d "
                        "at time %"PRId64"\n",
                        c->node, stat->pkt_seq, stat->tx_time);
        }
    }
...
/* STATS: cleanup handler for the statistics */
int stats_cleanup(call_t *c, void *das_entry) {
    struct _app_stat *stat = (struct _app_stat*) das_entry;
...
int setnode(call_t *c, void *params) {
    ...
    /* STATS: create statistics database for the node */
    nodedata->stats_id = stats_create(c, stats_cleanup);
...
int unsetnode(call_t *c) {
    ...
    /* STATS: Process the statistics before the node frees 
     * its private data */ 
    stats_process(nodedata->stats_id, stats_handler);
...
void tx(call_t *c) {
    ...
    /* STATS: add tx statistics */
    struct _app_stat *stat = malloc(sizeof(struct _app_stat));
    memset(stat, 0, sizeof(struct _app_stat));
    stat->tx_time = get_time();
    stat->pkt_seq = payload->seq;
    stats_add(nodedata->stats_id, (void *) stat);
...
void rx(call_t *c, packet_t *packet) {
    ...
    /* STATS: add rx statistics */
    struct _app_stat *stat = malloc(sizeof(struct _app_stat));
    memset(stat, 0, sizeof(struct _app_stat));
    stat->rx_time = get_time();
    stat->pkt_seq = payload->seq;
    stats_add(nodedata->stats_id, (void *) stat);
